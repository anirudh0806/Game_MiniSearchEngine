...
function games(search) {
  ...
  axios
    .get('https://api.rawg.io/api/games?key=540e382efbae471299617c777115b8ec&search=' + search)
    .then((res) => {
      console.log(res);
...
function desc(id) {
  document.getElementById('results').innerHTML = '';
  document.getElementById('desc').innerHTML = '';
  axios.get('https://api.rawg.io/api/games/'+id+'?key=540e382efbae471299617c777115b8ec').then((res) => {
    console.log(res);
    let desc = res.data;
    let platforms = '';
    let genres = '';
